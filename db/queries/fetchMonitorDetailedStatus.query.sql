...
SELECT
  ...
  m.is_active,
  m.is_public,
  month_to_date_incidents.accumulated_downtime AS month_to_date,
  rolling_month_incidents.accumulated_downtime AS rolling_month,
  rolling_year_incidents.accumulated_downtime AS rolling_year,
  100 * (
    1 - (
      EXTRACT(EPOCH FROM COALESCE(month_to_date_incidents.accumulated_downtime, INTERVAL '0 second')) 
      / 
